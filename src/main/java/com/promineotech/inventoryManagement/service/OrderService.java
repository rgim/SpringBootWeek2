...
public class OrderService {
	...
	public Order submitNewOrder(Set<Long> productIds, Long customerId) throws Exception {
		try {
			...
			Optional <Customer> possibleCustomer;
			Customer foundCustomer;
			possibleCustomer = customerRepo.findById(customerId);
			foundCustomer = possibleCustomer.get();
			
			Order order = initializeNewOrder(productIds, foundCustomer);
	...
	public Order cancelOrder(Long orderId) throws Exception {
		try {
			...
			Order foundOrder;
			possibleOrder = repo.findById(orderId);
			foundOrder = possibleOrder.get();
			
			foundOrder.setStatus(OrderStatus.CANCELLED);
			return repo.save(foundOrder);
		} catch (Exception e) {
